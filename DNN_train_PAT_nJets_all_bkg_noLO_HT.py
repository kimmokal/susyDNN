...
for mass_point in mass_point_list:
	...
	train_path = data_dir+'train_set_all_bkg_nJets_parametric_woLO_HT_'+str(mass_point)+'.root'
	test_path = data_dir+'test_set_all_bkg_nJets_parametric_woLO_HT_'+str(mass_point)+'.root'
	...
	dM_input = Input(shape = (1, ))
	advPremodel = keras.layers.concatenate([model(inputs), dM_input])
	...
	advmodel = Model(inputs=[inputs, dM_input], outputs=[advPremodel])
	...
	DRf = Model(inputs=[inputs, dM_input], outputs=[model(inputs), advmodel([inputs, dM_input])])
	...
	DfR = Model(inputs=[inputs, dM_input], outputs=[advmodel([inputs, dM_input])])
	...
	save_name_preadv = 'susyDNN_PAT_model_nJets_all_bkg_woLO_HT_lambda'+str(lam)+'_'+str(mass_point)
	...
	if lam >= 0.0:
		...
		for i in range(num_epochs):
			...
			if lam >= 0.0:
				...
				DfR.fit([bkg_x, bkg_x.dM_Go_LSP], nJets_binned_bkg, batch_size=batch_size, epochs=1, verbose=1)

	# Save the model
	save_path = '/work/kimmokal/susyDNN/models/'
	save_name = 'susyDNN_PAT_model_nJets_all_bkg_woLO_HT_lambda'+str(lam)+'_'+str(mass_point)
